...
class fpn_rpn_outputs(nn.Module):
    ...
    def __init__(self, dim_in, spatial_scales):
        ...
        num_anchors = len(cfg.FPN.RPN_ASPECT_RATIOS) # (0.5, 1, 2) -> 3
    ...
    def forward(self, blobs_in, im_info, roidb=None):
        ...
        for lvl in range(k_min, k_max + 1):
            ...
            fpn_rpn_conv = F.relu(self.FPN_RPN_conv(bl_in), inplace=True) # [12, 256, 56, 56]
            fpn_rpn_cls_score = self.FPN_RPN_cls_score(fpn_rpn_conv) # [12, 3, 56, 56]
            fpn_rpn_bbox_pred = self.FPN_RPN_bbox_pred(fpn_rpn_conv) # [12, 12, 56, 56]
            return_dict['rpn_cls_logits_fpn' + slvl] = fpn_rpn_cls_score
            return_dict['rpn_bbox_pred_fpn' + slvl] = fpn_rpn_bbox_pred

            if cfg.MODEL.FASTER_RCNN: # true
                ...
                rois_blobs.append(fpn_rpn_rois) # (1273, 5)
                score_blobs.append(fpn_rpn_roi_probs) # (1273, 1)
        ...
        if cfg.MODEL.FASTER_RCNN: # true
            ...
            # ['rois', 'rois_fpn2', 'rois_fpn3', 'rois_fpn4', 'rois_fpn5', 'rois_idx_restore_int32']
...
def fpn_rpn_losses(**kwargs):
    ...
    for lvl in range(cfg.FPN.RPN_MIN_LEVEL, cfg.FPN.RPN_MAX_LEVEL + 1):
        ...
        rpn_bbox_inside_weights_fpn = kwargs['rpn_bbox_inside_weights_wide_fpn' + slvl][:, :, :h, :w]
        rpn_bbox_outside_weights_fpn = kwargs['rpn_bbox_outside_weights_wide_fpn' + slvl][:, :, :h, :w]
