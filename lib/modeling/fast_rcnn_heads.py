...
class fast_rcnn_embedding_ls(nn.Module):
...
class roi_2mlp_head(nn.Module):
    ...
    def forward(self, x, rpn_ret, rois_name='rois', use_relu=True):
        ...
        # Generalized_RCNN.roi_feature_transform
        x = self.roi_xform(
            x, rpn_ret,
            blob_rois=rois_name,
        ...
        batch_size = x.size(0) # [512, 256, 7, 7]
        x = F.relu(self.fc1(x.view(batch_size, -1)), inplace=True) # [512, 1024]
        if use_relu:
...
class roi_2avg_pool(nn.Module):
    ...
    def forward(self, x, rpn_ret, rois_name='rois', use_relu=True):
        ...
        batch_size = x.size(0) # [512, 256, 7, 7]
        x = x.view(batch_size, self.dim_in, -1).mean(dim=2)
        if use_relu:
