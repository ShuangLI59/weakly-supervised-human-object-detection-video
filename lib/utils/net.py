...
def load_ckpt(model, ckpt):
    ...
    ckpt_new = {k: v for k, v in ckpt.items() if k in model_dict and model_dict[k].shape==ckpt[k].shape}
    model_dict.update(ckpt_new) 
    model.load_state_dict(model_dict)
    
    ckpt_samename_diffsize = {k: v for k, v in ckpt.items() if k in model_dict and model_dict[k].shape!=ckpt[k].shape}
    print('--------------------------------------------------------------------------------')
    print('ckpt_samename_diffsize: ', list(ckpt_samename_diffsize.keys()))
    print('--------------------------------------------------------------------------------')
