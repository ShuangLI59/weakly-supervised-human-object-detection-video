...
def log_stats(stats, misc_args, is_training):
    ...
    if is_training:
        ...
        if hasattr(misc_args, 'epoch'):
            ...
            lines += "[%s][%s][Step %d / %d]\n" % (
                misc_args.run_name, misc_args.cfg_filename, stats['val_iter'], cfg.SOLVER.MAX_ITER)

        lines += "\t\tval_loss: %.6f, val_time: %.6f, val_eta: %s\n" % (
            stats['val_loss'], stats['val_time'], stats['val_eta']
        )
        if stats['val_metrics']:
            lines += "\t\t" + ", ".join("%s: %.6f" % (k, v) for k, v in stats['val_metrics'].items()) + "\n"
        if stats['val_head_losses']:
            lines += "\t\t" + ", ".join("%s: %.6f" % (k, v) for k, v in stats['val_head_losses'].items()) + "\n"
    # if cfg.RPN.RPN_ON:
