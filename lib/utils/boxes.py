...
def ls_clearn_box(img_info, bbox):
    ...
    new_bbox_split = []
    for tem in bbox_split:
        if (tem!='' and tem!='[' and tem!=']'):
            if '[' in tem:
                tem = tem[1:]
            if ']' in tem:
                tem = tem[:-1]
            new_bbox_split.append(float(tem))
            
    ## vg has 5 dim
    if len(new_bbox_split)==5:
        new_bbox_split = new_bbox_split[:4]
    
    ## oi 
    if dataset_name=='oi':
        new_bbox_split = [new_bbox_split[1] * im_width, new_bbox_split[0] * im_height, new_bbox_split[3] * im_width, new_bbox_split[2] * im_height]

    new_bbox_split = [int(tem) for tem in new_bbox_split]
    

    # if dataset_name=='coco':
    #     box = torch.tensor(box).to(torch.int64)
    #     top_left, bottom_right = box[:2].tolist(), box[2:].tolist()
    # elif dataset_name=='oi':
    #     im_height, im_width, _ = image.shape
    #     box = [box[1] * im_width, box[0] * im_height, box[3] * im_width, box[2] * im_height]
    #     box = torch.tensor(box).to(torch.int64)
    #     top_left, bottom_right = box[:2].tolist(), box[2:].tolist()
    # elif dataset_name=='vg':
    #     box = torch.tensor(box).to(torch.int64)
    #     top_left, bottom_right = box[:2].tolist(), box[2:-1].tolist()


    return new_bbox_split
